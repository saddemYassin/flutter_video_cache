...
/// Manages the download operations efficiently and seamlessly.
///
/// This class orchestrates the downloading process, handling tasks such as starting, pausing,
/// resuming, and canceling downloads. It also provides progress tracking and callback registration
...
class DownloadManager {
    ...
    _mp4FileDownloader.registerCallback(({required String url,required String taskId, required double progress, required int total,required int status}) {
      ...
      if (taskId != null) {
        ...
        if (_tasks[taskId] != null) {
          _tasks[taskId]!.updateStatus(taskStatus,'in callback downloader manager');
  ...
  Future<void> pauseDownload(String key) async {
    ...
    await _mp4FileDownloader.pauseDownload(_tasks[_accessMap[key]!]!.url);
  }
  ...
  Future<void> resumeDownload(String key) async {
    ...
    await _mp4FileDownloader.resumeDownload(_tasks[_accessMap[key]!]!.url);
  }
  ...
  Future<void> cancelDownload(String key) async {
    ...
    await _mp4FileDownloader.cancelDownload(_tasks[_accessMap[key]!]!.url);
  }
