...
class AppPlayerWithControls extends StatefulWidget {
  ...
  @override
  AppPlayerWithControlsState createState() => AppPlayerWithControlsState();
}

class AppPlayerWithControlsState extends State<AppPlayerWithControls> {
  ...
  late AppVideoController _appController;
  ...
  void initState() {
    super.initState();
    _appController = widget.controller;
    _aspectRatio = widget.aspectRatio;
    _appController.controller!.addListener(listener);
  }

  void listener() {
    if (!mounted) return;
    //
    if (_appController.controller!.value.isInitialized) {
      final oPosition = _appController.controller!.value.position;
      final oDuration = _appController.controller!.value.duration;
      ...
      setState(() {
        numberOfCaptions = _appController.controller!.value.spuTracksCount;
        numberOfAudioTracks = _appController.controller!.value.audioTracksCount;
      });
      // update recording blink widget
      if (_appController.controller!.value.isRecording && _appController.controller!.value.isPlaying) {
      ...
      if (isRecording != _appController.controller!.value.isRecording) {
        setState(() => isRecording = _appController.controller!.value.isRecording);
        if (!isRecording) {
          widget.onStopRecording?.call(_appController.controller!.value.recordPath);
        }
      }
    }
  }
              ...
              children: [
                Wrap(
                  children: [
                    ...
                    IconButton(
                      color: Colors.white,
                      icon: _appController.controller!.value.isRecording
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Size: ${_appController.controller!.value.size.width.toInt()}'
                            'x${_appController.controller!.value.size.height.toInt()}',
                      ...
                      Text(
                        'Status: ${_appController.controller!.value.playingState.toString().split('.')[1]}',
                        textAlign: TextAlign.center,
                        overflow: TextOverflow.ellipsis,
                        style:
                        const TextStyle(color: Colors.white, fontSize: 10),
          ...
          child: ColoredBox(
            ...
            child: Stack(
              ...
              children: <Widget>[
                Center(
                  child: VlcPlayer(
                    controller: _appController.controller!,
                ...
                ControlsOverlay(controller: _appController.controller!),
              ],
            ),
          ),
        ),
          ...
          child: ColoredBox(
            ...
            child: Row(
              children: [
                IconButton(
                  color: Colors.white,
                  icon: _appController.controller!.value.isPlaying
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Slider(
                          ...
                          max: !validPosition
                              ? 1.0
                              : _appController.controller!.value.duration.inSeconds.toDouble(),
                          onChanged:
  ...
  @override
  void dispose() {
    _appController.controller!.removeListener(listener);
    super.dispose();
  }
  ...
  Future<void> _cyclePlaybackSpeed() async {
    ...
    return _appController
        .controller!.setPlaybackSpeed(playbackSpeeds.elementAt(playbackSpeedIndex));
  }
  ...
  void _setSoundVolume(double value) {
    ...
    _appController.controller!.setVolume(volumeValue.toInt());
  }

  Future<void> _togglePlaying() async {
    _appController.controller!.value.isPlaying
        ? await _appController.pause()
        : await _appController.play();
  }

  Future<void> _toggleRecording() async {
    if (!_appController.controller!.value.isRecording) {
      final saveDirectory = await getTemporaryDirectory();
      await _appController.controller!.startRecording(saveDirectory.path);
    } else {
      await _appController.controller!.stopRecording();
    }
  }
  ...
  void _onSliderPositionChanged(double progress) {
    ...
    _appController.controller!.setTime(sliderValue.toInt() * Duration.millisecondsPerSecond);
  }

  Future<void> _getSubtitleTracks() async {
    if (!_appController.controller!.value.isPlaying) return;

    final subtitleTracks = await _appController.controller!.getSpuTracks();
    //
    if (subtitleTracks.isNotEmpty) {
      ...
      if (selectedSubId != null) {
        await _appController.controller!.setSpuTrack(selectedSubId);
      }
    }
  }
  ...
  Future<void> _getAudioTracks() async {
    ...
    final audioTracks = await _appController.controller!.getAudioTracks();
    //
    if (audioTracks.isNotEmpty) {
      ...
      if (selectedAudioTrackId != null) {
        await _appController.controller!.setAudioTrack(selectedAudioTrackId);
      }
    }
  }

  Future<void> _getRendererDevices() async {
    final castDevices = await _appController.controller!.getRendererDevices();
    //
    if (castDevices.isNotEmpty) {
      ...
      if (selectedCastDeviceName != null) {
        await _appController.controller!.castToRenderer(selectedCastDeviceName);
      }
    } else {
      if (!mounted) return;
  ...
  Future<void> _createCameraImage() async {
    final snapshot = await _appController.controller!.takeSnapshot();
